...
const COOKIE = $.isNode() ? require("./qhydCOOKIE") : ``;
let status;
...
if ($.isNode() && process.env.YQNB_yqnbHEADER) {
    ...
    for (let i = 1; i <= qhydCount; i++) {
        ...
        if (COOKIE.datas.find(item => item.key === `qhydheader${op}`)) {

            qhydheaderArr.push(COOKIE.datas.find(item => item.key === `qhydheader${op}`).val);

            qdydbodyArr.push(COOKIE.datas.find(item => item.key === `qhydbody${op}`).val);
